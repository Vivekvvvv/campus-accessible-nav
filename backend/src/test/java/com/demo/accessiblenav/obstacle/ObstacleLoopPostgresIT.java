...
/**
 * Real DB regression suite (PostgreSQL/PostGIS) for the obstacle loop v2.
 *
 * Run with: mvn -Pit verify
 */
public class ObstacleLoopPostgresIT extends PostgresITBase {
    ...
    @Test
    void migrations_shouldCreatePartialUniqueIndexForActiveEffects() {
        List<String> indexes = jdbcTemplate.queryForList(
                "select indexname from pg_indexes where schemaname = current_schema() and tablename = 't_obstacle_effect'",
                String.class
        );
        assertThat(indexes).contains("uk_effect_edge_active_true");
        assertThat(indexes).doesNotContain("uk_effect_edge_active");
    }
    ...
    void approve_shouldKeepEffectHistory_andRevokePreviousActiveEffect() {
        ...
        createB.setReason("B");
        createB.setSubmitterLat(23.2753000);
        createB.setSubmitterLng(113.2003000);
        Long reportBId = reportService.createReport(createB).getId();
        assertThat(reportBId).isNotEqualTo(reportAId);
        ...
        assertThat(allEffects.stream().filter(e -> e.getEdge().getId().equals(edge.getId())).count())
